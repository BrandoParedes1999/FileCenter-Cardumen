...
<div class="fc-wrapper">
    ...
    <div class="fc-main">
        ...
        <div class="fc-content">
            ...
            <div class="fc-page-header">
                <div>
                    ...
                    <div style="font-size:13px;color:var(--fc-text-muted);margin-top:2px">
                        ...
                        @else
                            Historial de tus solicitudes de acceso a otras empresas
                        @endif
                    </div>
                </div>
            ...
            {{-- Filtros de status con contadores --}}
            @php
                ...
                $badgeStyle = [
                    'Pendiente' => 'background:#d97706;color:#fff',
                    'Aprobado'  => 'background:#059669;color:#fff',
                    'Rechazado' => 'background:#dc2626;color:#fff',
                ];
            ...
            <div class="fc-filter-bar" style="margin-bottom:20px;gap:6px">
                <a href="{{ route('solicitudes.index') }}"
                   ...
                   style="display:inline-flex;align-items:center;gap:6px">
                    Todas
                    @if($totalGeneral > 0)
                    <span style="font-size:10px;font-weight:700;padding:1px 6px;border-radius:20px;background:{{ $filtroActual === 'todas' ? 'rgba(255,255,255,.25)' : 'rgba(99,102,241,.12)' }};color:{{ $filtroActual === 'todas' ? '#fff' : '#6366f1' }}">
                        {{ $totalGeneral }}
                    </span>
                    @endif
                </a>
                ...
                <a href="{{ route('solicitudes.index', ['status' => $val]) }}"
                   ...
                   style="display:inline-flex;align-items:center;gap:6px">
                    {{ $label }}
                    @if(($conteos[$val] ?? 0) > 0)
                    <span style="font-size:10px;font-weight:700;padding:1px 6px;border-radius:20px;{{ $filtroActual === $val ? 'background:rgba(255,255,255,.25);color:#fff' : $badgeStyle[$val] }}">
                        {{ $conteos[$val] }}
                    </span>
                    @endif
                </a>
            ...
            <div class="fc-empty">
                ...
                @if($filtroActual !== 'todas')
                    <div class="fc-empty-title">Sin solicitudes {{ strtolower($statusOpts[$filtroActual] ?? $filtroActual) }}</div>
                    <div class="fc-empty-sub">No hay solicitudes con estado "{{ $statusOpts[$filtroActual] ?? $filtroActual }}" en este momento.</div>
                ...
                <table class="fc-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            {{-- Col 1: Solicitante (admin) o Empresa objetivo (empleado) --}}
                            @if($esAdmin)
                            <td>
                            ...
                            <td>
                                ...
                                <div style="display:flex;align-items:center;gap:9px">
                                    ...
                                    <div>
                                        <div style="font-size:12px;font-weight:600;color:var(--fc-text)">
                                            {{ Str::limit($sol->archivo->nombre_original ?? '(eliminado)', 32) }}
                                        </div>
                                ...
                                <div style="display:flex;align-items:center;gap:9px">
                                    ...
                                    <div>
                                        <div style="font-size:12px;font-weight:600;color:var(--fc-text)">
                                            {{ Str::limit($sol->carpeta->nombre ?? '(eliminada)', 32) }}
                                        </div>
                            ...
                            {{-- Col 3: Tipo de acceso --}}
                            <td>
                                <span class="fc-badge" style="background:rgba(99,102,241,.08);color:#4f46e5;font-size:10px">
                                    {{ $sol->tipo_acceso ?? '—' }}
                                </span>
                            </td>
                            ...
                            {{-- Col 4: Estado --}}
                            <td>
                                <span class="fc-badge" style="background:{{ $sc['bg'] }};color:{{ $sc['color'] }};font-size:10px">
                                    {{ $sol->status }}
                                </span>
                            </td>
